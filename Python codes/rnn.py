...
for i in range(LSTM_TIMESTEPS, len(train_data)):
	lstm_train_data.append(train_data[i - LSTM_TIMESTEPS : i,:])
	lstm_train_labels.append(train_labels[i - 1,:])
for i in range(LSTM_TIMESTEPS, len(test_data)):
	lstm_test_data.append(test_data[i - LSTM_TIMESTEPS : i,:])
	lstm_test_labels.append(test_labels[i - 1,:])
print(lstm_train_data[1])

np_train_data = np.array(lstm_train_data)
np_train_labels = np.array(lstm_train_labels)
np_test_data = np.array(lstm_test_data)
np_test_labels = np.array(lstm_test_labels)
...
for k in range(MAXITER):
	...
	print(model.summary())
	hist = model.fit(np_train_data, np_train_labels, epochs=EPOCHS, batch_size=256)

	results = model.evaluate(np_test_data, np_test_labels)
